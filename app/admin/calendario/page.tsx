...
export default function ProgramarCalendario() {
  ...
  const [jornada, setJornada] = useState(''); // <-- NUEVO ESTADO
  ...
  const fetchData = async () => {
    ...
    const { data: part } = await supabase
      ...
      .gt('fecha', ahora)
      .order('jornada', { ascending: true }) // <-- ORDENAR POR JORNADA
  ...
  const agendar = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!localId || !visitaId || !fecha || !jornada) return alert("Completa todos los campos");
    ...
    const { error } = await supabase.from('partidos').insert([
      { 
        ...
        fecha: fechaAjustada,
        jornada: parseInt(jornada), // <-- GUARDAR JORNADA
        estado: 'programado' 
      }
    ]);
  ...
  // LÓGICA PARA AGRUPAR PARTIDOS POR JORNADA
  const partidosPorJornada = partidosProgramados.reduce((acc: any, partido) => {
    const j = partido.jornada || "Sin Jornada";
    if (!acc[j]) acc[j] = [];
    acc[j].push(partido);
    return acc;
  }, {});
              ...
              <div key={numJornada} className="space-y-3">
                ...
                {partidosPorJornada[numJornada].map((p: any) => (
                  <div key={p.id} className="bg-zinc-900/30 border border-zinc-800 p-5 rounded-3xl flex justify-between items-center group hover:border-zinc-600 transition-all">
                    <div>
                      <p className="text-[10px] text-zinc-400 font-mono font-black mb-1 uppercase tracking-widest">
